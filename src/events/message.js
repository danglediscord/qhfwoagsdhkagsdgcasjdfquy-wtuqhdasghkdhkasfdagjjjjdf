...
module.exports = async (client, message) => {
  ...
  if (!message.guild && cmd.conf.guildOnly) return client.embed.send(message, { desc: 'Bu komut, özel mesajlar aracılığıyla kullanılamaz. Lütfen bu komutu bir loncada çalıştırın.' })

  // Is the bot currently under going maintenence?
  if (client.config.botMaintenance && message.author.id !== client.config.ownerID) return client.embed.send(message, { desc: 'Bot şu anda yalnızca bot sahibi tarafından çalıştırılabilir. rahatsızlıktan dolayı özür dileriz.' })
  ...
  client.permLevel(client, message).then(permLevel => {
    if (permLevel >= client.levelCache[cmd.conf.permLevel]) {
      ...
      client.embed.send(message, {
        code: true,
        desc: 'Bu komutu çalıştırma izniniz yok.',
        fields: [
          {
            name: 'Have',
